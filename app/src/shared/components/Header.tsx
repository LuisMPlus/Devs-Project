  ...
  return (
    ...
    >
      ...
      <nav className="flex items-center gap-6 lg:gap-8 text-sm md:text-base font-medium tracking-wide" style={{ color: 'var(--color-text)' }}>
        <Link to="/" className="hover:text-(--color-primary) transition-colors">Inicio</Link>
        <Link to="/carreras" className="hover:text-(--color-secondary) transition-colors">Carreras</Link>
      </nav>
    </header>
  )
}
